  ...
  if (created) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        ...
        <h1 className="mt-4 text-[22px] font-semibold text-[#1d1d1f]">Request sent</h1>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-6 inline-flex items-center gap-1.5 rounded-full border border-[#d2d2d7] px-4 py-2 text-[13px] font-semibold text-[#1d1d1f]"
        >
    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      <div className="border-b border-[#e8e8ed] bg-white/80 backdrop-blur">
        <div className="mx-auto max-w-3xl px-4 py-4">
          ...
          <h1 className="mt-2 text-[22px] font-semibold tracking-tight text-[#1d1d1f]">
            Book {proName}
          </h1>
      ...
      <div className="mx-auto max-w-3xl space-y-4 px-4 py-6">
        <section className="rounded-[22px] border border-[#e8e8ed] bg-white p-4 shadow-sm sm:p-5">
          ...
          <div className="mb-2 grid grid-cols-7 gap-1 text-center text-[11px] font-semibold uppercase text-[#86868b]">
            {WEEKDAYS.map((d) => (
              <div key={d}>{d}</div>
            ))}
          </div>
          ...
          {loading ? (
            <p className="py-8 text-center text-[13px] text-[#86868b]">Loading…</p>
          ) : (
                      ...
                      disabled
                        ...
                        : selectedDate === key
                          ...
                          : isBlocked
                            ...
                            : 'border-[#f0f0f2] text-[#1d1d1f] hover:border-[#d2d2d7]'
                    }`}
                  >
                    {d.getDate()}
                  </button>
        ...
        <section className="rounded-[22px] border border-[#e8e8ed] bg-white p-4 shadow-sm sm:p-5">
          <h3 className="text-[15px] font-semibold text-[#1d1d1f]">Time</h3>
          {!selectedDate ? (
            <p className="mt-2 text-[13px] text-[#86868b]">Select a date first</p>
          ) : openHours.length === 0 ? (
            <p className="mt-2 text-[13px] text-[#86868b]">No open slots this day</p>
          ) : (
          ...
          <label className="mt-4 block">
            ...
            <textarea
              className="mt-1 w-full rounded-xl border border-[#d2d2d7] bg-[#fafafa] px-3 py-2.5 text-[14px] outline-none focus:border-[#1d1d1f] focus:bg-white"
              rows={3}
              value={notes}
          ...
          <button
            ...
            className="mt-4 w-full rounded-full bg-[#1d1d1f] py-3 text-[14px] font-semibold text-white disabled:opacity-50"
          >
            {!user
              ? 'Log in to request'
              : submitting
