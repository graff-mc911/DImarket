...
export function ProCalendar() {
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <CalendarDays className="mx-auto h-10 w-10 text-[#86868b]" />
        <p className="mt-4 text-[15px] text-[#1d1d1f]">Sign in to manage your calendar</p>
        <button
          type="button"
          className="mt-6 rounded-full bg-[#1d1d1f] px-5 py-2.5 text-[13px] font-semibold text-white"
  ...
  if (!isPro) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <p className="text-[15px] text-[#1d1d1f]">Booking calendar is for professionals.</p>
        <button
          type="button"
          className="mt-6 rounded-full border border-[#d2d2d7] px-5 py-2.5 text-[13px] font-semibold"
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]">
      <div className="border-b border-[#e8e8ed] bg-white/80 backdrop-blur">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {gcal ? (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-[#d2d2d7] px-4 py-2 text-[12px] font-semibold"
              >
      ...
      <div className="mx-auto grid max-w-6xl gap-4 px-4 py-6 lg:grid-cols-[1.4fr_1fr]">
        ...
        <section className="rounded-[22px] border border-[#e8e8ed] bg-white p-4 shadow-sm sm:p-5">
          <div className="mb-4 flex items-center justify-between">
            <button
              type="button"
              className="rounded-full p-2 hover:bg-[#f5f5f7]"
              onClick={() => setCursor(new Date(year, month - 1, 1))}
            >
              <ChevronLeft className="h-5 w-5" />
            </button>
            <h2 className="text-[16px] font-semibold text-[#1d1d1f]">{monthLabel}</h2>
            <button
              type="button"
              className="rounded-full p-2 hover:bg-[#f5f5f7]"
              onClick={() => setCursor(new Date(year, month + 1, 1))}
          ...
          <div className="mb-2 grid grid-cols-7 gap-1 text-center text-[11px] font-semibold uppercase tracking-wide text-[#86868b]">
            {WEEKDAYS.map((d) => (
              <div key={d}>{d}</div>
            ))}
          </div>
          ...
          {loading ? (
            <p className="py-10 text-center text-[13px] text-[#86868b]">Loading…</p>
          ) : (
                      ...
                      !inMonth
                        ...
                        : isBlocked
                          ...
                          : selected === key
                            ...
                            : 'border-[#f0f0f2] hover:border-[#d2d2d7]'
                    }`}
          ...
          {selected ? (
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl bg-[#fafafa] p-3">
              <span className="text-[13px] font-semibold text-[#1d1d1f]">{selected}</span>
              <button
                type="button"
                disabled={busy}
        ...
        <div className="space-y-4">
          <section className="rounded-[22px] border border-[#e8e8ed] bg-white p-4 shadow-sm">
            <h3 className="text-[15px] font-semibold text-[#1d1d1f]">
              Pending requests ({pending.length})
            </h3>
            <div className="mt-3 space-y-2">
              {pending.length === 0 ? (
                <p className="text-[13px] text-[#86868b]">No pending requests</p>
              ) : (
                pending.map((b) => (
                  <div key={b.id} className="rounded-xl border border-[#f0f0f2] p-3">
                    <p className="text-[14px] font-semibold text-[#1d1d1f]">{b.customer_name}</p>
                    ...
                    {b.notes ? (
                      <p className="mt-1 text-[12px] text-[#6e6e73]">{b.notes}</p>
                    ) : null}
                    <div className="mt-2 flex gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-full bg-[#f5f5f7] px-3 py-1.5 text-[12px] font-semibold text-[#1d1d1f]"
                      >
          ...
          <section className="rounded-[22px] border border-[#e8e8ed] bg-white p-4 shadow-sm">
            <h3 className="text-[15px] font-semibold text-[#1d1d1f]">Appointments</h3>
            <div className="mt-3 max-h-[420px] space-y-2 overflow-y-auto">
              {upcoming.length === 0 ? (
                <p className="text-[13px] text-[#86868b]">No upcoming appointments</p>
              ) : (
                upcoming.map((b) => (
                  <div key={b.id} className="rounded-xl border border-[#f0f0f2] p-3">
