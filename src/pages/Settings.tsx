...
async function getDeleteAccountErrorDetail(error: unknown): Promise<string> {
...
export function Settings() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const detail = await getDeleteAccountErrorDetail(error)
      const isMissingFunction =
        detail.includes('404') ||
        detail.toLowerCase().includes('not found') ||
        detail.toLowerCase().includes('non-2xx')

      setFeedback({
        type: 'error',
        text: isMissingFunction
          ? t('settings.error.deleteAccountNotDeployed')
          : detail
            ? `${t('settings.error.deleteAccount')}: ${detail}`
            : t('settings.error.deleteAccount'),
      })
    }
  }
