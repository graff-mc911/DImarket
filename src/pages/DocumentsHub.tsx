    ...
    <div className="layout-page-content py-8 pb-24 lg:pb-8">
      <div className="mx-auto max-w-4xl">
        <header className="mb-6">
          ...
          <p className="mt-3 inline-flex items-center gap-1.5 rounded-none bg-[#f3f0ea] px-3 py-1.5 text-xs text-[color:var(--ink-900)]">
            <MapPin className="h-3.5 w-3.5 text-[color:var(--label-ink)]" aria-hidden />
            {t('docs.location.context')}: <strong>{locationLabel}</strong>
          </p>
          {jurisdiction.countryCode ? null : (
            <p className="mt-2 text-xs text-[color:var(--label-ink)]">{t('docs.location.pickHint')}</p>
          )}
        </header>
            ...
            {DOCUMENTS_SUBCATEGORY_ORDER.map((sub) => (
              <li key={sub.slug}>
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-none border border-[rgba(148,163,184,0.22)] bg-white px-4 py-3 text-left transition hover:border-[#007185]/40"
                >
                  <span className="text-xl" aria-hidden>
                    {sub.icon}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block font-semibold text-[color:var(--ink-900)]">{tStored(t, sub.titleKey)}</span>
                    <span className="block text-xs text-[color:var(--label-ink)]">{tStored(t, sub.descriptionKey)}</span>
                  </span>
                  <ChevronRight className="h-4 w-4 shrink-0 text-[color:var(--label-ink)]" aria-hidden />
                </button>
              </li>
            ))}
          </ul>
        ...
        <h2 className="mb-3 text-lg font-bold text-[color:var(--ink-900)]">
          {subcategory ? t('docs.list.title') : t('docs.list.featured')}
        </h2>

        {docs.length === 0 ? (
          <p className="rounded-none border border-dashed border-[rgba(148,163,184,0.35)] px-4 py-8 text-center text-sm text-[color:var(--label-ink)]">
              ...
              <li key={doc.id}>
                ...
                >
                  <div className="flex items-start gap-3">
                    <FileText className="mt-0.5 h-5 w-5 shrink-0 text-[color:var(--ink-900)]" aria-hidden />
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm text-[color:var(--label-ink)] line-clamp-2">
        ...
        <p className="mt-6 text-xs leading-5 text-[color:var(--label-ink)]">{t('docs.disclaimer.short')}</p>
      </div>
    </div>
  )
}
