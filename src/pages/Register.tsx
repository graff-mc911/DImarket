...
type RegisterRole = UserRole | 'advertiser'
...
export function Register() {
  ...
  const ROLE_OPTIONS = [
    {
      ...
      description: t('register.roleClientDesc'),
    },
    {
      role: 'professional' as RegisterRole,
      icon: <HardHat className="h-6 w-6" />,
      title: t('register.roleProfessional'),
      description: t('register.roleProfessionalDesc'),
    },
    {
      role: 'company' as RegisterRole,
      icon: <Building2 className="h-6 w-6" />,
      title: t('register.roleCompany'),
      description: t('register.roleCompanyDesc'),
    },
    {
      role: 'advertiser' as RegisterRole,
      icon: <Megaphone className="h-6 w-6" />,
      title: t('register.roleAdvertiser'),
      description: t('register.roleAdvertiserDesc'),
    },
  ]
  ...
  const handleRegister = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const locationText = [city, region, country].filter(Boolean).join(', ') || null
    ...
    try {
      ...
      setError(alreadyRegistered ? t('register.alreadyRegistered') : message)
    } finally {
      setLoading(false)
    }
  }
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {ROLE_OPTIONS.map((option) => (
                    <button
                      ...
                      style={{
                        borderColor:
                          selectedRole === option.role ? 'var(--accent-700)' : 'var(--glass-border)',
                        background:
                          selectedRole === option.role
                            ? 'rgba(199,138,96,0.12)'
                            : 'rgba(255,255,255,0.4)',
                        color:
                          selectedRole === option.role ? 'var(--accent-700)' : 'var(--ink-600)',
                      }}
                    >
                      <div
