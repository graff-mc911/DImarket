  ...
  if (!stats.count) {
    return (
      <div className="rounded-none border border-[rgba(148,163,184,0.22)] bg-[#fafafa] px-4 py-6 text-center">
        <p className="text-[14px] font-semibold text-[color:var(--ink-900)]">No reviews yet</p>
        <p className="mt-1 text-[13px] text-[color:var(--label-ink)]">Be the first to leave feedback.</p>
      </div>
    )
  }
      ...
      <div className="flex flex-wrap items-end gap-6">
        <div>
          ...
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-[40px] font-semibold leading-none tracking-tight text-[color:var(--ink-900)]">
              {stats.average.toFixed(1)}
            </span>
            <span className="text-[14px] text-[color:var(--label-ink)]">/ 5</span>
          </div>
          ...
          {([5, 4, 3, 2, 1] as const).map((star) => {
            ...
            return (
              <div key={star} className="flex items-center gap-2 text-[12px]">
                <span className="w-3 font-semibold text-[color:var(--ink-900)]">{star}</span>
                ...
                <span className="w-6 text-right text-[color:var(--label-ink)]">{n}</span>
              </div>
            )
          })}
        </div>
