  ...
  const panel = (
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-[#f0f0f2] px-4 py-3">
        <div>
          <p className="text-[14px] font-semibold text-[color:var(--ink-900)]">{t('notifications.title')}</p>
        ...
        <div className="flex flex-wrap justify-end gap-2">
          ...
          <button
            type="button"
            className="text-[11px] font-semibold text-[color:var(--label-ink)]"
            ...
            >
              <span
                className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-full ${
                  !n.is_read ? 'bg-indigo-100 text-indigo-700' : 'bg-[#f3f0ea] text-[color:var(--label-ink)]'
                }`}
              >
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-wide text-[color:var(--label-ink)]">
                    {typeLabel(n.type, t)}
                  </span>
                  {!n.is_read ? (
                ...
                <span className="mt-0.5 block text-[13px] font-semibold text-[color:var(--ink-900)]">
                  {n.title}
                </span>
                <span className="mt-0.5 line-clamp-2 block text-[12px] text-[color:var(--label-ink)]">
                  {n.body}
                </span>
      ...
      <div className="flex items-center justify-between border-t border-[#f0f0f2] px-4 py-2.5">
        ...
        {!compact ? null : (
          <button
            type="button"
            className="text-[12px] font-semibold text-[color:var(--label-ink)]"
            onClick={() => {
              setOpen(false)
  ...
  if (!compact) {
    return (
      <div className="px-4 py-8">
        <h1 className="mb-4 text-[22px] font-semibold text-[color:var(--ink-900)]">{t('notifications.centerTitle')}</h1>
        {panel}
      </div>
    )
  }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute -right-0.5 -top-0.5 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#ff9900] px-1 text-[10px] font-bold text-[color:var(--ink-900)]">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
  ...
  if (!user) {
    return (
      <div className="px-4 py-16 text-center">
        <p className="text-[14px] text-[color:var(--label-ink)]">{t('notifications.signIn')}</p>
        <button
          type="button"
