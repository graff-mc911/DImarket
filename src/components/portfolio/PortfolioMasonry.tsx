  ...
  if (!filtered.length) {
    return (
      <div className="rounded-none border border-dashed border-[rgba(148,163,184,0.35)] bg-white/60 px-6 py-14 text-center">
        ...
        <p className="mt-3 text-[14px] font-semibold text-[color:var(--ink-900)]">No portfolio items yet</p>
              ...
              >
                {item.media_type === 'before_after' && item.before_url && item.after_url ? (
                  ...
                  <div className="relative bg-[#0b0b0f]">
                    ...
                    <span className="absolute inset-0 flex items-center justify-center">
                      <span className="rounded-full bg-white/90 p-3 shadow">
                        <Play className="h-5 w-5 text-[color:var(--ink-900)]" />
                      </span>
                    </span>
                  </div>
                ) : cover ? (
                  ...
                  <div className="flex h-36 items-center justify-center bg-[#f3f0ea]">
                    <Award className="h-8 w-8 text-[color:var(--label-ink)]" />
                  </div>
                )}
              </button>
              ...
              <div className="p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="truncate text-[14px] font-semibold text-[color:var(--ink-900)]">{item.title}</p>
                    <p className="mt-0.5 text-[11px] font-medium uppercase tracking-wide text-[color:var(--label-ink)]">
                ...
                {item.description ? (
                  <p className="mt-2 line-clamp-3 text-[12px] leading-relaxed text-[color:var(--label-ink)]">
                    {item.description}
                  </p>
                ) : null}
                ...
                <div className="mt-3 flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full bg-[#f3f0ea] px-2.5 py-1.5 text-[12px] font-semibold text-[color:var(--label-ink)] hover:bg-[rgba(148,163,184,0.22)]"
                  >
          ...
          >
            ...
            {lightbox.media_type === 'before_after' && lightbox.before_url && lightbox.after_url ? (
              <div className="grid gap-2 sm:grid-cols-2">
                <div>
                  <p className="mb-1 text-[11px] font-bold uppercase text-[color:var(--label-ink)]">Before</p>
                  <img src={lightbox.before_url} alt="Before" className="w-full rounded-none object-cover" />
                </div>
                <div>
                  <p className="mb-1 text-[11px] font-bold uppercase text-[color:var(--label-ink)]">After</p>
            ...
            <div className="mt-4">
              <h3 className="text-[18px] font-semibold text-[color:var(--ink-900)]">{lightbox.title}</h3>
              <p className="mt-1 text-[12px] font-medium uppercase tracking-wide text-[color:var(--label-ink)]">
                {t(portfolioCategoryKey(lightbox.category_slug) as never)} ·{' '}
                {lightbox.media_type.replace('_', ' ')}
              </p>
              {lightbox.description ? (
                <p className="mt-3 text-[14px] leading-relaxed text-[color:var(--label-ink)]">{lightbox.description}</p>
              ) : null}
