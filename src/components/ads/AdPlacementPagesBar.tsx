      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {pages.map((page) => {
          ...
          return (
            ...
            >
              <span className="block text-[11px] font-extrabold leading-tight text-[color:var(--ink-900)]">
                {t(page.labelKey)}
                {count > 0 ? (
                  <span className="ml-1 font-bold text-[#6366f1]">({count})</span>
                ) : null}
              </span>
              <span className="mt-0.5 block font-mono text-[9px] leading-snug text-[color:var(--label-ink)]">
                {page.route}
              </span>
            </button>
          )
