 ...
 */
export function GeoSearchFilters({ value, onChange, className = '' }: GeoSearchFiltersProps) {
  const { t } = useApp()
  ...
  return (
    <div className={className}>
      ...
      <div className="amazon-filter-group">
        ...
        {gpsError ? <p className="mt-1 text-xs text-red-600">{gpsError}</p> : null}
        {(value.country || value.city || value.fromGps) && (
          <button type="button" className="amazon-link mt-2 text-xs" onClick={clearGeo}>
            {t('geo.clear')}
          </button>
        )}
      </div>
