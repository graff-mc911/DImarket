...
function markdownToHtml(md: string): string {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('> ')) {
      if (!inBlockquote) {
        out.push('<blockquote style="margin:12px 0;padding:12px 16px;border-left:4px solid rgba(148,163,184,0.35);background:#f3f0ea;font-size:13px;color:#6f665d">')
        inBlockquote = true
      }
    ...
    if (line.startsWith('# ')) {
      ...
      out.push('<hr style="border:none;border-top:1px solid rgba(148,163,184,0.22);margin:20px 0"/>')
    } else if (line.trim()) {
      out.push(`<p style="margin:8px 0;font-size:14px;line-height:1.6;color:#2f2a24">${escapeHtml(line).replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/\*(.+?)\*/g, '<em>$1</em>')}</p>`)
    }
  }
...
<style>
  ...
  .dimarket-doc-meta { margin-top:40px;padding-top:16px;border-top:2px solid #2f2a24;font-size:11px;color:#6f665d; }
  .dimarket-doc-disclaimer { margin-top:8px;font-size:10px;line-height:1.5;color:#8a8178; }
</style>
</head>
<body>
  <div class="sheet">
    <div style="border-bottom:2px solid #2f2a24;padding-bottom:16px;margin-bottom:24px">
      <div style="font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#8a8178">DImarket · Official document</div>
      <h1 style="margin:8px 0 0;font-size:26px;letter-spacing:-0.02em">${escapeHtml(input.title)}</h1>
      <p style="margin:6px 0 0;font-size:12px;color:#8a8178">${escapeHtml(footerMeta.jurisdiction)} · Version ${escapeHtml(footerMeta.version)}</p>
    </div>
    ...
    <p style="margin-top:24px;font-size:10px;color:#8a8178;text-align:center">Generated with DImarket · dimarket.app</p>
  </div>
