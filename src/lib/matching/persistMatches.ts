...
import { dispatchToProfessionals } from '../aiDispatcher'
import { filterNotifyCandidates } from './notifyMatches'
...
export async function runMatchingForListing(
  ...
  let notifiedCount = 0
  if (notifyIds.length) {
    // AI Dispatcher: description + photos + address + estimate + deadline
    const dispatched = await dispatchToProfessionals(listingId, notifyIds)
    notifiedCount = dispatched.notified
  }
