/** Довідковий каталог областей/міст. Об'єднується з geo_catalog; реєстрації додають нові точки. */
export type ReferenceGeoData = Record<string, Record<string, string[]>>

export const REFERENCE_GEO_DATA: Record<string, Record<string, string[]>> = {
  // EUROPE
  'Ukraine': {
    'Вінницька':           ['Вінниця', 'Жмеринка'],
    'Волинська':           ['Луцьк', 'Ковель'],
    'Дніпропетровська':    ['Дніпро', 'Кривий Ріг', "Кам'янське", 'Нікополь'],
    'Донецька':            ['Донецьк', 'Маріуполь', 'Краматорськ'],
    'Житомирська':         ['Житомир', 'Бердичів'],
    'Закарпатська':        ['Ужгород', 'Мукачеве', 'Хуст'],
    'Запорізька':          ['Запоріжжя', 'Мелітополь', 'Бердянськ'],
    'Івано-Франківська':   ['Івано-Франківськ', 'Коломия', 'Калуш'],
    'Київська':            ['Київ', 'Бровари', 'Бориспіль', 'Ірпінь', 'Біла Церква'],
    'Кіровоградська':      ['Кропивницький', 'Олександрія'],
    'Луганська':           ['Луганськ', 'Сєвєродонецьк'],
    'Львівська':           ['Львів', 'Дрогобич', 'Стрий', 'Самбір'],
    'Миколаївська':        ['Миколаїв', 'Первомайськ'],
    'Одеська':             ['Одеса', 'Ізмаїл', 'Чорноморськ', 'Южне'],
    'Полтавська':          ['Полтава', 'Кременчук'],
    'Рівненська':          ['Рівне', 'Дубно'],
    'Сумська':             ['Суми', 'Конотоп'],
    'Тернопільська':       ['Тернопіль', 'Чортків'],
    'Харківська':          ['Харків', 'Ізюм', 'Лозова'],
    'Херсонська':          ['Херсон', 'Нова Каховка'],
    'Хмельницька':         ['Хмельницький', "Кам'янець-Подільський"],
    'Черкаська':           ['Черкаси', 'Умань'],
    'Чернівецька':         ['Чернівці'],
    'Чернігівська':        ['Чернігів', 'Ніжин'],
  },
  'Poland': {
  ...
  'Germany': {
    'Baden-Württemberg':      ['Stuttgart', 'Karlsruhe', 'Freiburg', 'Mannheim', 'Heidelberg'],
    ...
    'Niedersachsen':          ['Hanover', 'Braunschweig', 'Osnabrück', 'Oldenburg'],
    'North Rhine-Westphalia': ['Cologne', 'Düsseldorf', 'Dortmund', 'Essen', 'Bonn', 'Bielefeld'],
...
export function referenceGeoToRows(): Array<{ country: string; region: string; city: string }> {
  const rows: Array<{ country: string; region: string; city: string }> = []
  for (const [country, regions] of Object.entries(REFERENCE_GEO_DATA)) {
