 ...
 * tab switches, and short reloads. No secrets / File blobs — media by URL only.
 */
...
export function readOwnerAdFormDraft(ownerId: string): OwnerAdFormDraft | null {
  try {
    // localStorage: survives leave→return to another site in the same browser profile
    const raw =
      localStorage.getItem(OWNER_AD_FORM_DRAFT_KEY) ??
      sessionStorage.getItem(OWNER_AD_FORM_DRAFT_KEY)
    if (!raw) return null
...
export function writeOwnerAdFormDraft(
  ...
  try {
    if (!draft.formOpen) {
      localStorage.removeItem(OWNER_AD_FORM_DRAFT_KEY)
    ...
    localStorage.setItem(OWNER_AD_FORM_DRAFT_KEY, raw)
    sessionStorage.setItem(OWNER_AD_FORM_DRAFT_KEY, raw)
  } catch {
    /* quota / private mode */
  }
}
