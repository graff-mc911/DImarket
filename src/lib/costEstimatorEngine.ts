...
export { estimatorGeoMultiplier, vatRateForCountry } from './costEstimatorGeo'
...
export function buildFullCostEstimateLocal(state: EstimatorState): FullCostEstimate {
  ...
  const extra = featureExtraEur(
    ...
    state.location.city,
    state.includeMaterials !== false,
    type.laborShare,
  )
  if (extra > 0) factors.push('Selected project features')

  const laborMid = roundEuro(base * type.laborShare + extra * type.laborShare)
  const materialsMid = state.includeMaterials === false
    ? 0
    : roundEuro(base * (1 - type.laborShare) * 0.82 + extra * (1 - type.laborShare))
