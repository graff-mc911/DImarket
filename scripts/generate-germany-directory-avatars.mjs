#!/usr/bin/env node
/**
 * Generate original DImarket avatars for Germany directory profiles,
 ...
 *
 * Usage:
 *   VITE_SUPABASE_ANON_KEY=... node scripts/generate-germany-directory-avatars.mjs
 */
...
const seedPath = resolve(root, 'data/directory/germany-directory-nationwide.json')
const outDir = resolve(root, 'data/directory')
const tmpDir = resolve(outDir, '.germany-avatars-tmp')
...
writeFileSync(
  resolve(outDir, 'germany-directory-avatars-report.json'),
...
const sql = `-- Germany directory avatars backfill (generated DImarket art, not third-party photos)
...
writeFileSync(resolve(outDir, 'germany-directory-avatars-backfill.sql'), sql)
writeFileSync(
  resolve(root, 'supabase/migrations/20260805090000_backfill_germany_directory_avatars.sql'),
  sql,
)

// Merge into directoryAvatars.ts client map (keep Spain + add Germany)
...
writeFileSync(
  resolve(outDir, 'GERMANY_DIRECTORY_AVATARS.md'),
