#!/usr/bin/env node
/**
 * Build Germany-wide masters + companies directory seed.
 * Usage: node scripts/build-germany-directory-seed.mjs
 */
import { writeFileSync, mkdirSync } from 'fs'
import { resolve, dirname } from 'path'
import { fileURLToPath } from 'url'
import { GERMANY_DIRECTORY_NATIONWIDE } from './germany-directory-nationwide-data.mjs'
...
function normalize(raw) {
  ...
  return {
    ...
    languages: raw.languages?.length ? raw.languages : ['de'],
    preferred_language: raw.languages?.[0] || 'de',
    ...
    seo: {
      ...
      meta_description: `Find ${raw.full_name} in ${raw.city}, Germany on DImarket. Request a quote for ${raw.categories.slice(0, 3).join(', ').toLowerCase()} services.`,
      keywords,
    },
    sources: raw.sources || [],
    import: {
      claimable: true,
      source: 'germany-directory-nationwide',
    },
  }
}
...
const seen = new Set()
const businesses = []
for (const raw of GERMANY_DIRECTORY_NATIONWIDE) {
  if (raw.country_code !== 'DE' && raw.country !== 'Germany') continue
...
const payload = {
  ...
  source:
    'Curated Germany-wide public masters and companies from company websites / Impressum. Factual fields only; original DImarket bios; no reviews/ratings/photos copied. Includes map coordinates.',
  schema: {
...
const outPath = resolve(outDir, 'germany-directory-nationwide.json')
writeFileSync(outPath, JSON.stringify(payload, null, 2) + '\n')
writeFileSync(
  resolve(outDir, 'GERMANY_DIRECTORY_IMPORT.md'),
