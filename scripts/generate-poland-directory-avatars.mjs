#!/usr/bin/env node
/**
 * Generate original DImarket avatars for Poland directory profiles,
 ...
 *
 * Usage:
 *   VITE_SUPABASE_ANON_KEY=... node scripts/generate-poland-directory-avatars.mjs
 */
...
const seedPath = resolve(root, 'data/directory/poland-directory-nationwide.json')
const outDir = resolve(root, 'data/directory')
const tmpDir = resolve(outDir, '.poland-avatars-tmp')
...
writeFileSync(
  resolve(outDir, 'poland-directory-avatars-report.json'),
...
const sql = `-- Poland directory avatars backfill (generated DImarket art, not third-party photos)
...
writeFileSync(resolve(outDir, 'poland-directory-avatars-backfill.sql'), sql)
writeFileSync(
  resolve(root, 'supabase/migrations/20260805100000_backfill_poland_directory_avatars.sql'),
  sql,
)

// Merge into directoryAvatars.ts client map (keep Spain/Germany + add Poland)
...
writeFileSync(
  resolve(outDir, 'POLAND_DIRECTORY_AVATARS.md'),
