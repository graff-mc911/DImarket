#!/usr/bin/env node
/**
 * Build Slovakia-wide masters + companies directory seed.
 * Usage: node scripts/build-slovakia-directory-seed.mjs
 */
import { writeFileSync, mkdirSync } from 'fs'
import { resolve, dirname } from 'path'
import { fileURLToPath } from 'url'
import { SLOVAKIA_DIRECTORY_NATIONWIDE } from './slovakia-directory-nationwide-data.mjs'
...
function normalize(raw) {
  ...
  return {
    ...
    languages: raw.languages?.length ? raw.languages : ['sk'],
    preferred_language: raw.languages?.[0] || 'sk',
    ...
    seo: {
      ...
      meta_description: `Find ${raw.full_name} in ${raw.city}, Slovakia on DImarket. Request a quote for ${raw.categories.slice(0, 3).join(', ').toLowerCase()} services.`,
      keywords,
    },
    sources: raw.sources || [],
    import: {
      claimable: true,
      source: 'slovakia-directory-nationwide',
    },
  }
}
...
const seen = new Set()
const businesses = []
for (const raw of SLOVAKIA_DIRECTORY_NATIONWIDE) {
  if (raw.country_code !== 'SK' && raw.country !== 'Slovakia') continue
...
const payload = {
  ...
  source:
    'Curated Slovakia-wide public masters and companies from company websites. Factual fields only; original DImarket bios; no reviews/ratings/photos copied. Includes map coordinates.',
  schema: {
...
const outPath = resolve(outDir, 'slovakia-directory-nationwide.json')
writeFileSync(outPath, JSON.stringify(payload, null, 2) + '\n')
writeFileSync(
  resolve(outDir, 'SLOVAKIA_DIRECTORY_IMPORT.md'),
