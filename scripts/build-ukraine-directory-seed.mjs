#!/usr/bin/env node
/**
 * Build Ukraine-wide masters + companies directory seed.
 * Usage: node scripts/build-ukraine-directory-seed.mjs
 */
import { writeFileSync, mkdirSync } from 'fs'
import { resolve, dirname } from 'path'
import { fileURLToPath } from 'url'
import { UKRAINE_DIRECTORY_NATIONWIDE } from './ukraine-directory-nationwide-data.mjs'
...
function normalize(raw) {
  ...
  return {
    ...
    languages: raw.languages?.length ? raw.languages : ['uk'],
    preferred_language: raw.languages?.[0] || 'uk',
    ...
    seo: {
      ...
      meta_description: `Find ${raw.full_name} in ${raw.city}, Ukraine on DImarket. Request a quote for ${raw.categories.slice(0, 3).join(', ').toLowerCase()} services.`,
      keywords,
    },
    sources: raw.sources || [],
    import: {
      claimable: true,
      source: 'ukraine-directory-nationwide',
    },
  }
}
...
const seen = new Set()
const businesses = []
for (const raw of UKRAINE_DIRECTORY_NATIONWIDE) {
  if (raw.country_code !== 'UA' && raw.country !== 'Ukraine') continue
...
const payload = {
  ...
  source:
    'Curated Ukraine-wide public masters and companies from company websites / Impressum. Factual fields only; original DImarket bios; no reviews/ratings/photos copied. Includes map coordinates.',
  schema: {
...
const outPath = resolve(outDir, 'ukraine-directory-nationwide.json')
writeFileSync(outPath, JSON.stringify(payload, null, 2) + '\n')
writeFileSync(
  resolve(outDir, 'UKRAINE_DIRECTORY_IMPORT.md'),
