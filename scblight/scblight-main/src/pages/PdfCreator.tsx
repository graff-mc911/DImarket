...
/** OCR-панель (вбудована, без окремого файлу PdfOcrPanel.tsx) */
function InlineOcrPanel({ onOpenInEditor }: { onOpenInEditor: (doc: UniversalDocument) => void }) {
  ...
  const isImage = (file: File) =>
    file.type.startsWith('image/') || /\.(jpg|jpeg|png|webp|gif|bmp|tiff)$/i.test(file.name);
    ...
    try {
      ...
      if (isImage(file)) {
        ...
        for (let p = 1; p <= pageCount; p += 1) {
          ...
          if (pageText) parts.push(`--- Сторінка ${p} ---\n${pageText}`);
        }
        setProgress(100);
      }
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="text-center">
        ...
        <p className="text-white/50 text-sm mt-1">Фото, скан або PDF → текст у редакторі</p>
      </div>
      <button
        type="button"
...
export default function PdfCreator() {
  ...
  return (
    <div className="min-h-screen pt-20 pb-28 px-4 md:px-6 mx-auto max-w-7xl">
      ...
      {/* OCR */}
      {hubMode === 'ocr' && <InlineOcrPanel onOpenInEditor={openInEditor} />}
    </div>
  );
}
